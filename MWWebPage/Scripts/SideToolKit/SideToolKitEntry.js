...
function SideToolKitEntry() {
    this.pf = new Profile();
    this.SideToolKits = new Array("Pf", "Sp", "Mw");

    this.SideToolKitsSetup();

    this.wardrobe = new Wardrobe();
    this.stylepaltte = new StylePalette();
}

SideToolKitEntry.prototype.SideToolKitsSetup = function () {

    var self = this;

    // for side tool kit show&hide
    $.each(this.SideToolKits, function () {
        ...
        $('#' + elem + '-btn').hover(function () {
            ...
            $.each(self.SideToolKits, function () {
    ...
    $('#menu-background').click(function (event) {
        var opened_pane = self.getOpenedPaneString();
        if (opened_pane == null) return;
        var des_elem = self.pointInElementRect(event.clientX, event.clientY, $('#' + opened_pane + '-Pane-Top-Button'));
        if (des_elem)
            $(des_elem).click();
    });
...
SideToolKitEntry.prototype.getOpenedPaneString = function () {
    var opened_pane = null;
    $.each(this.SideToolKits, function () {
        var inner = new String(this);
        if ($('#' + inner + '-Pane').hasClass("SideToolKitActive"))
...
SideToolKitEntry.prototype.pointInElementRect = function (cx, cy, group) {
    var reVal = null
